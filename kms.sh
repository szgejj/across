...
install_main() {
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://raw.githubusercontent.com/szgejj/across/master/kms; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://raw.githubusercontent.com/szgejj/across/master/kms-debian; then
            echo -e "[${red}Error:${plain}] Failed to download KMS Server script."
            exit 1
        fi
    else
    ...
    cd "${cur_dir}" || exit
    git clone https://github.com/szgejj/vlmcsd.git > /dev/null 2>&1
